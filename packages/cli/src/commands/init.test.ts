...
describe('initCommand', () => {
  it('creates starter files in the current directory', async () => {
    ...
    const helloFlow = await fs.readFile(
      path.join(tempDir, 'flows', 'hello.ts'),
      'utf-8',
    );
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Created flows\/hello\.ts/),
    );
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Created payload\.json/),
    );
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Created \.env\.example/),
    );
  });
  ...
  it('overwrites existing files when force is true', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Updated flows\/hello\.ts/),
    );
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Updated payload\.json/),
    );
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Updated \.env\.example/),
    );
  });
